		...
		if ($RadioProvision.Checked)
		{
			...
			{				          
				...
				switch($RetVal.ReturnValue)
				{
					...
					{
					#Output to Logfile
					$TextStrings.LogShareCreated -f (Get-Date -F G), $ShareName | Out-File -FilePath $Logfile -append
					} 
					"2"
					{
		...
		else
		{
			...
			foreach ($Share in $DelShares)
			{
				...
				switch($RetVal.ReturnValue)
				{
					...
					{
						#Output to Logfile
						$TextStrings.LogShareDeleted -f (Get-Date -F G), $Share | Out-File -FilePath $Logfile -append
					}
					"2"
					{
