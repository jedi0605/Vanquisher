...
$BtnRefresh.add_Click({Get-AllAdapterStatus($BtnRefresh)})
#~~< Button10 >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
$gbStaticIP.Location = New-Object System.Drawing.Point(28, 104)
$gbStaticIP.Size = New-Object System.Drawing.Size(359, 105)
$gbStaticIP.TabIndex = 3
$gbStaticIP.TabStop = $false
$gbStaticIP.Text = $TextStrings.UseStaticIP
#~~< LblIPAddress >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
$Panel2.Location = New-Object System.Drawing.Point(14, 207)
...
$DNSStaticRadioButton.Location = New-Object System.Drawing.Point(0, 32)
...
$GroupBox2.Location = New-Object System.Drawing.Point(28, 242)
$GroupBox2.Size = New-Object System.Drawing.Size(359, 73)
$GroupBox2.TabIndex = 6
$GroupBox2.TabStop = $false
$GroupBox2.Text = $TextStrings.UseStaticDNS
#~~< TxtBoxAlernateDNS >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
$TxtBoxAlernateDNS = New-Object System.Windows.Forms.TextBox
$TxtBoxAlernateDNS.Location = New-Object System.Drawing.Point(184, 44)
...
$TxtBoxPreferredDNS.Location = New-Object System.Drawing.Point(184, 17)
...
$LblAlernateDNS.Location = New-Object System.Drawing.Point(26, 47)
...
$LblPreferredDNS.Location = New-Object System.Drawing.Point(26, 20)
...
#~~< lblAdapterWarning >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
$lblDNSAdapterWarning = New-Object System.Windows.Forms.Label
$lblDNSAdapterWarning.Font = New-Object System.Drawing.Font("Tahoma", 8.25, [System.Drawing.FontStyle]::Bold, [System.Drawing.GraphicsUnit]::Point, ([System.Byte](0)))
$lblDNSAdapterWarning.Location = New-Object System.Drawing.Point(8, 320)
$lblDNSAdapterWarning.Size = New-Object System.Drawing.Size(350, 28)
$lblDNSAdapterWarning.TabStop = $false
$lblDNSAdapterWarning.Text = $TextStrings.AdapterWarning
...
$panelIP.Controls.Add($lblDNSAdapterWarning)
...
function OK
{
	if($panelIP.visible -eq $True)
	{
		...
		{
			# handle specially if it is a virtual adapter
			if($Adapter.ComponentId -eq "vms_mp")
			{
				try
				{
					foreach ($VMAdapter in (Get-VMNetworkAdapter -ManagementOs -ErrorAction Stop))
					{
						if($Adapter.DeviceId -eq $VMAdapter.DeviceID)
						{
							if($TxtBoxName.Text -ne $VMAdapter.Name)
							{
								Rename-VMNetworkAdapter -ManagementOS -Name $VMAdapter.Name -NewName $TxtBoxName.Text
								$TextStrings.VAdapterRenamed -f (Get-Date -F G), $VMAdapter.Name, $TxtBoxName.Text | Out-File -FilePath $Logfile -append
							}
							break
						}
					}
				...
				{
					ErrorForm ($TextStrings.NoVRename -f $VMAdapter.Name, $TxtBoxName.Text, $_.Exception.Message)
					$TextStrings.NoVRename -f (Get-Date -F G), $VMAdapter.Name, $TxtBoxName.Text, $_.Exception.Message | Out-File -FilePath $Logfile -Append
				}
			}
			else
			{
				try
				{
					Rename-NetAdapter -Name $Adapter.Name -NewName $TxtBoxName.Text -ErrorAction Stop
					$TextStrings.AdapterRenamed -f (Get-Date -F G), $Adapter.Name, $TxtBoxName.Text | Out-File -FilePath $Logfile -append
				}
				catch [Exception]
				{
					ErrorForm ($TextStrings.NoRename -f $Adapter.Name, $TxtBoxName.Text, $_.Exception.Message)
					$TextStrings.NoRename -f (Get-Date -F G), $Adapter.Name, $TxtBoxName.Text, $_.Exception.Message | Out-File -FilePath $Logfile -Append
				}
			}
			$Adapter = Get-NetAdapter -InterfaceIndex $Adapter.InterfaceIndex
		}
		...
		if($DHCPRadioButton.Checked -eq $true)
		{
			...
			{
				# Gateways must be removed first or behavior is unpredictable
				...
				$TextStrings.LogDHCPEnabled -f (Get-Date -F G), $Adapter.Name | Out-File -FilePath $Logfile -append
				Do
				{
					Start-Sleep -Seconds 5	# Need to wait a bit before refreshing or the updated information is misleading
					$IPTest = Get-NetIPAddress -InterfaceAlias $Adapter.Name -ErrorAction SilentlyContinue
				} While ($IPTest.Length -eq 0)
			}
		}
		else
		{
			...
			$TextStrings.LogDHCPDisabled -f (Get-Date -F G), $Adapter.Name | Out-File -FilePath $Logfile -append
			try
			{
				# Gateways must be removed first or behavior is unpredictable
				Remove-NetRoute -InterfaceAlias $Adapter.Name -Confirm:$false -ErrorAction SilentlyContinue
				...
				if($TxtBoxDefaultGateway.Text.Length -gt 0)
				{
					...
					$TextStrings.IPManual -f (Get-Date -F G), $Adapter.Name, $NewIP, $DottedSubnet, $TxtBoxDefaultGateway.Text | Out-File -FilePath $Logfile -append
				}
				else
				{
					New-NetIPAddress -InterfaceAlias $Adapter.Name -IPAddress $NewIP -PrefixLength $Prefix -ErrorAction Stop -Confirm:$false
					$TextStrings.IPManual -f (Get-Date -F G), $Adapter.Name, $NewIP, $DottedSubnet, "" | Out-File -FilePath $Logfile -append
				}
			}
			catch [Exception]
			{
				...
				# rollback any adapter renames or there will be problems
				if ($Adapter.Name -ne $ListNetworkAdap.SelectedItem)
				{
					Rename-NetAdapter -Name $Adapter.Name -NewName $ListNetworkAdap.SelectedItem
				}
				# skip the refresh so the user can just fix any problems and not have to re-enter all info
				return
			}
		}
		# DNS Settings
		...
		if ($DNSDHCPRadioButton.Checked -eq $true)
		{
			...
			else
			{
				...
				$TextStrings.DNSResetToDHCP -f (Get-Date -F G), $Adapter.Name | Out-File -FilePath $Logfile -append
			}
			Get-AllIPStatus ($Adapter.Name)
			return
		}
		...
		# if manual DNS is dotted, then the intent is to replace positions 1 and 2 in some fashion
		# positions 3+ aren't visible here so ensure they aren't damaged or lost
		if($DNSServerArray.Count -gt 0)
		{
			$DNSServerArray[0] = "" # clean out DNS entry #1
			if($DNSServerArray.Count -gt 1)
			{
				$DNSServerArray[1] = "" # clean out DNS entry #2
			}
		}
		# check for and remove duplicates of entered items (ex: if user is trying to move #3 to #1 by entering it in the #1 slot)
		if(($DNSServerArray.Count -gt 0) -and ($DNS1.Text.Length -gt 0))
		{
			do
			{
				$FoundIndex = $DNSServerArray.IndexOf($DNS1)
				if ($FoundIndex -ne -1)
				{ $DNSServerArray[$FoundIndex] = "" }
			} while ($FoundIndex -ne -1)
		}
		if(($DNSServerArray.Count -gt 0) -and ($DNS2.Text.Length -gt 0))
		{
			do
			{
		...
		# Set slots 1 and 2 according to manual entry
		if($DNSServerArray.Length -gt 0)
		{ $DNSServerArray[0] = $DNS1 }
		else { $DNSServerArray += $DNS1 }
		if($DNSServerArray.Length -gt 1)
		{ $DNSServerArray[1] = $DNS2 }
		else { $DNSServerArray += $DNS2 }
		# Remove any zero-length entries from the array
		$FinalDNSArray = @()
		foreach($DNSEntry in $DNSServerArray)
		{
			if($DNSEntry.Length -gt 0)
			{ $FinalDNSArray += $DNSEntry }
		}
		# set resultant DNS server array or clear if none are left
		if($FinalDNSArray.Count -gt 0)
		{
			try
			{
				Set-DNSClientServerAddress -InterfaceAlias $Adapter.Name -ServerAddress $FinalDNSArray
				$TextStrings.DNSManual -f (Get-Date -F G), $Adapter.Name | Out-File -FilePath $Logfile -append
			}
			catch [Exception]
			{
				ErrorForm ($_.Exception.Message)
	...
	else #	$PanelAllDNS.visible = $True
	{
		...
		Get-AllIPStatus ($TxtBoxName.Text)
	}
}
#---------------------------------------------------
...
#*****************************************************************************************************************************************************************
function Get-AllIPStatus
{
	param ([String]$SpecificAdapterName="")
	$IPSettingsform.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
	$ListNetworkAdap.Items.Clear()
	$DNSListNetworkAdap.Items.Clear()
	...
	if($SpecificAdapterName.Length -gt 0)
	{
		$SpecificIndex = $ListNetworkAdap.FindString($SpecificAdapterName)
		if($SpecificIndex -gt -1)
		{
			$ListNetworkAdap.set_SelectedItem($SpecificAdapterName)
		}
	}
	$IPSettingsform.Cursor = [System.Windows.Forms.Cursors]::Default
}
#*****************************************************************************************************************************************************************
function Get-AllAdapterStatus
{
	$IPSettingsform.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
...
function GetIp([string]$InAdapter)
{
	...
	if($Adapter.ComponentId -eq "vms_mp")
	{
		...
		{
			# Tripped when the system doesn't have the Hyper-V PowerShell module enabled
			$DisplayName = $Adapter.Name
			$ChkBoxName.Enabled = $false
		}
	}
	else
	{ $DisplayName = $Adapter.Name }
	$IPInt = Get-NetIPInterface -InterfaceIndex $Adapter.InterfaceIndex -AddressFamily IPv4
	$TxtBoxName.Text = $DisplayName
...
function AddDNS
{
	...
	$PanelDNS.location = New-Object System.Drawing.Size(55,180)
}
#*****************************************************************************************************************************************************************
function OKDNSAdd
{
	$PanelAllDNS.enabled = $true
	if(!($ListDNS.Items.Contains($TxtDNSadd.Text)))
...
function EnableNic
{
	if($ChkdDisabled.CheckedItems.Count -gt 0)
	{
		$IPSettingsform.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
		foreach ($AdapterName in $ChkdDisabled.CheckedItems)
		{
			Enable-NetAdapter -Name $AdapterName -Confirm:$false
			$TextStrings.AdapterEnabled -f (Get-Date -F G), $AdapterName | Out-File -FilePath $Logfile -append
		}
		Start-Sleep -Milliseconds 5000	# if an adapter is enabled here but disconnected, need a delay for the system to detect that
		Get-AllIPStatus
		Get-AllAdapterStatus
		$IPSettingsform.Cursor = [System.Windows.Forms.Cursors]::Default
	}
...
function DisableNic
{
	...
	foreach ($AdapterName in $ChkdEnabled.CheckedItems)
	{
		...
		$TextStrings.AdapterDisabled -f (Get-Date -F G), $AdapterName | Out-File -FilePath $Logfile -append
	}
	Get-AllIPStatus
	Get-AllAdapterStatus
	$IPSettingsform.Cursor = [System.Windows.Forms.Cursors]::Default
