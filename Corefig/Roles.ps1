...
#endregion
#~~< lblListBoxHeader >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
$lblListBoxHeader = New-Object System.Windows.Forms.Label
$lblListBoxHeader.Font = New-Object System.Drawing.Font("Tahoma", 8.25, [System.Drawing.FontStyle]::Bold, [System.Drawing.GraphicsUnit]::Point, ([System.Byte](0)))
$lblListBoxHeader.Location = New-Object System.Drawing.Point(62, 19)
$lblListBoxHeader.Size = New-Object System.Drawing.Size(210, 17)
$lblListBoxHeader.TabIndex = 69
$lblListBoxHeader.Text = $TextStrings.Installable
#~~< PctInstallable >~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
$panelMenu.Controls.Add($lblListBoxHeader)
...
function RestartNow
{
	...
	if($CommonLoaded -ne $true) {	. $ScriptDirectory\Common.ps1 }
	Message
...
function LblRemoveRolesClick($object)
{
	...
	$Rolesform.Invalidate
	$Rolesform.Update
	Get-Roles $False
	$lblListBoxHeader.Text = $TextStrings.Removable
...
function Get-Roles
{
	param([bool]$AvailableToAdd)
	$ListRoles.Items.Clear()
	if($AvailableToAdd)
	{
		$ButtonOK.Text = $TextStrings.AddRF
		$InstallStateCheck = "Disabled"
	#	This is for a possible future enhancement to show pending items
	#	$InstallPendingCheck = "EnablePending"
	}
	else
	{
		$ButtonOK.Text = $TextStrings.RemoveRF
		$InstallStateCheck = "Enabled"
	#	This is for a possible future enhancement to show pending items
	#	$InstallPendingCheck = "DisablePending"
	}
	foreach ($Feature in (Get-WindowsOptionalFeature -Online))
	{
		if($Feature.State -eq $InstallStateCheck)
		{
			$ListRoles.Items.Add($Feature.FeatureName)
		}
		#	This is for a possible future enhancement to show pending items
		#elseif($Feature.InstallState -eq $InstallPendingCheck)
		#{
		#	$ListRoles.Items.Add($Feature.DisplayName, [System.Windows.Forms.CheckState]::Indeterminate)
		#}
	}
}
...
function OK
{
	if($ListRoles.CheckedItems.Count -eq 0) { return }

	$ButtonOK.enabled = $False
	$ButtonCancel.enabled = $False
	$ListRoles.enabled = $False
	$RadioADD.enabled = $False
	$RadioRemove.enabled = $False
	$ProgressBar.Visible = $True
	$Rolesform.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
	$Installing = $RadioADD.Checked
	$ProgressBar.Value = 0
	$ProgressBar.Step = (100 / $ListRoles.CheckedItems.Count)
	if($Installing)
	{
		$ActionDescription = $TextStrings.InstallationOf
	}
	else
	{
		$ActionDescription = $TextStrings.RemovalOf
	}
	$TextStrings.LogChangeStarted -f (Get-Date -F G), $ActionDescription, $ListRoles.CheckedItems.Count | Out-File -FilePath $Logfile -append
	# One-at-a-time isn't the most efficient way, but there's no "AsJob" for these commands
	# so the screen would just go dead during long enable/disable cycles otherwise
	foreach ($RoleToChange in $ListRoles.CheckedItems)
	{
		try
		{
			if($Installing)
			{
				if((Enable-WindowsOptionalFeature -Online -All -FeatureName $RoleToChange -NoRestart -ErrorAction Stop).RestartNeeded)
				{ $RestartNeeded = $True }
				$TextStrings.LogProcessedInstall -f (Get-Date -F G), $RoleToChange | Out-File -FilePath $Logfile -append
			}
			else
			{
				if ((Disable-WindowsOptionalFeature -Online -FeatureName $RoleToChange -NoRestart -ErrorAction Stop).RestartNeeded)
				{ $RestartNeeded = $True }
				$TextStrings.LogProcessedRemoval -f (Get-Date -F G), $RoleToChange | Out-File -FilePath $Logfile -append
			}
		}
		catch [Exception]
		{
			$TextStrings.LogProcessError -f (Get-Date -F G), $RoleToChange, $_.Exception.Message | Out-File -FilePath $Logfile -append
			$Flag = $TextStrings.RFErrorTitle
			$Message = $TextStrings.RFErrorMessage -f $RoleToChange
			if($CommonLoaded -ne $true) {	. $ScriptDirectory\Common.ps1 }
			Message
			$btnOK.text = $TextStrings.Continue
			$btnClose.text = $TextStrings.Stop
			$pboxWarning.visible = $true
			$frmMessage.ShowDialog()
			if ($ReturnFlag -eq "NO")
			{
	...
	$TextStrings.LogChangeCompleted -f (Get-Date -F G), "$env:windir\Logs\Dism\dism.log" | Out-File -FilePath $Logfile -append
	$ButtonOK.enabled = $True
	$ButtonCancel.enabled = $True
	$ListRoles.enabled = $True
	$RadioADD.enabled = $True
	$RadioRemove.enabled = $True
	$ProgressBar.Visible = $False
	Get-Roles $Installing
